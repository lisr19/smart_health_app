...
//			self.wrapClass = '.timeline.timeline-horizontal.timeline-initialized'
		...
		build_dots: function () {
			...
			for (i = 0; i <= (self.get_count() - 1); i += 1) {
				...
				dot.append($('<li />').append(self.options.customPaging.call(this, self, itemDate,i)));
			}
	...
	$.fn.Timeline.options = {
		...
		// CONTROLS
		customPaging: function(slider, date,index) {
			return $('<button type="button" role="button" onclick="changeDateData('+index+')"/>').text(date);
		},
	};
